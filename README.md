...
If you want to add log correlation for ElasticAPM, be sure to have enabled it in
your initializer:

```ruby
Rails.application.configure do
  config.ecs_log_rails.log_correlation = true
end
```

Then include the module in `ApplicationJob`:
...
> [!IMPORTANT]
> The `EcsLogRails::ActiveJob::LogCorrelation` will register an `around_perform`
> callback. The callback must be added **after** the `ElasticAPM` transaction is
> already active. Since `around_perform` callbacks execute in FIFO order and
> since you're probably opening the transaction manually in an around callback
> (because `ElasticAPM` doesn't support `ActiveJob` out of the box), then you
> must `include` the module in the right order.
>
> ```ruby
> class ApplicationJob < ActiveJob::Base
>   around_perform do |job, block|
>     ElasticAPM.with_transaction(job.class.name, :background_jobs) do |_transaction|
>       block.call
>     end
>   end
>   include EcsLogRails::ActiveJob::LogCorrelation
...
`lograge_activejob.custom_options` is configured automatically by `ecs_log_rails`.
`ecs_log_rails` assumes to have full control over this configuration. Just leave
it alone.
